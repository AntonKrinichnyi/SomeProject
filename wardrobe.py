...
def glasses_fitting(frame, glasses=0):
    try:
        glasses_image = cv2.imread(f"static/{glasses}.png", cv2.IMREAD_UNCHANGED) # Завантажуемо зображення окулярів
    ...
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) # Перетворюємо кольорове зображення в чорно-біле 
    ...
    for rect in rects: # Проходимось по цім точкам циклом
        ...
        if glasses_image is not None:
            resized_glasses_rgb = cv2.resize(glasses_rgb,
                                            (glasses_width,
                                            int(glasses_image.shape[0] * glasses_width / glasses_image.shape[1])),
                                            interpolation=cv2.INTER_AREA)
            resized_glasses_alpha = cv2.resize(glasses_alpha,
                                            (glasses_width,
                                            int(glasses_image.shape[0] * glasses_width / glasses_image.shape[1])),
                                            interpolation=cv2.INTER_AREA)
            ...
            M = cv2.getRotationMatrix2D(center_of_glasses, -angle, 1.0) # Повертаємо їх на кут який ми розрахували раніше (кут має бути від'ємним або окуляри будуть повертатись не в ту сторону)
            ...
            """
            rotated_glasses_rgb = cv2.warpAffine(resized_glasses_rgb, M, (w, h), borderMode=cv2.BORDER_REPLICATE)
            rotated_glasses_alpha = cv2.warpAffine(resized_glasses_alpha, M, (w, h), borderMode=cv2.BORDER_CONSTANT, borderValue=0)
